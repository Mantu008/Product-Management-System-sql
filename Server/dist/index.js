...
app.post("/register", (req, resp) => __awaiter(void 0, void 0, void 0, function* () {
    ...
    try {
        ...
        config_1.default.query("SELECT * FROM users WHERE username = ?", [username], (err, result) => __awaiter(void 0, void 0, void 0, function* () {
            if (err) {
            ...
            config_1.default.query("INSERT INTO users SET ?", user, (err, result) => {
                if (err) {
                    console.error("Database error:", err);
                    return resp
                        .status(500)
                ...
                jsonwebtoken_1.default.sign({ data: respdata }, "mantu000", // Ensure this is set correctly
...
app.post("/login", (req, res) => __awaiter(void 0, void 0, void 0, function* () {
    ...
    try {
        config_1.default.query("SELECT * FROM users WHERE username = ? LIMIT 1", [username], (err, result) => __awaiter(void 0, void 0, void 0, function* () {
            if (err) {
                console.error("Database error:", err);
            ...
            jsonwebtoken_1.default.sign({ id: user.id, username: user.username }, "mantu000", // Ensure this is set correctly
            { expiresIn: "2h" }, (err, token) => {
                if (err) {
                    return res
                        .status(500)
...
app.post("/create-product", (req, resp) => __awaiter(void 0, void 0, void 0, function* () {
    const { username, name, productCode, hsn, selesPrice, purchasePrice, } = req.body;
    if (!username ||
        !name ||
        !productCode ||
        !hsn ||
    ...
    try {
        ...
        config_1.default.query("SELECT * FROM users WHERE username = ?", [username], (err, userResult) => {
            if (err) {
                console.error("Database error:", err);
                return resp
                    .status(500)
            ...
            config_1.default.query("SELECT * FROM products WHERE productCode = ?", [productCode], (err, productResult) => {
                if (err) {
                    console.error("Database error:", err);
                    return resp
                        .status(500)
                ...
                config_1.default.query("INSERT INTO products SET ?", product, (err, result) => {
                    if (err) {
                        console.error("Database error:", err);
                        return resp
                            .status(500)
...
// Read all products
app.post("/allproduct", verifyToken, (req, resp) => __awaiter(void 0, void 0, void 0, function* () {
...
app.post("/product/:id", verifyToken, (req, resp) => __awaiter(void 0, void 0, void 0, function* () {
    const { username } = req.body;
    ...
    try {
        ...
        config_1.default.query("SELECT * FROM users WHERE username = ?", [username], (err, userResult) => {
            ...
            config_1.default.query("SELECT * FROM products WHERE productCode = ?", [productId], (err, productResult) => {
                if (err) {
                    console.error("Database error:", err);
                    return resp
                        .status(500)
...
app.put("/update/:id", verifyToken, (req, resp) => __awaiter(void 0, void 0, void 0, function* () {
    const { username, name, productCode, hsn, selesPrice, purchasePrice, } = req.body;
    if (!username ||
        !name ||
        !productCode ||
        ...
        config_1.default.query("SELECT * FROM users WHERE username = ?", [username], (err, userResult) => {
            ...
            if (userResult[0].role === "admin") {
                ...
                config_1.default.query("SELECT * FROM products WHERE productCode = ?", [productId], (err, productResult) => {
                    ...
                    config_1.default.query("UPDATE products SET name = ?, productCode = ?, hsn = ?, selesPrice = ?, purchasePrice = ? WHERE productCode = ?", [name, productCode, hsn, selesPrice, purchasePrice, productId], (err, result) => {
                        if (err) {
                            console.error("Database error:", err);
                            return resp
                                .status(500)
                ...
                resp.status(403).json({
                    error: "You are not authorized to update product data. Only admins can.",
                });
            }
        });
    }
...
app.delete("/delete/:id", verifyToken, (req, resp) => __awaiter(void 0, void 0, void 0, function* () {
    const { username } = req.body;
    if (!username) {
        return resp.status(400).json({ error: "Missing required fields" });
    }
        ...
        config_1.default.query("SELECT * FROM users WHERE username = ?", [username], (err, userResult) => {
            ...
            if (userResult[0].role !== "admin") {
                return resp.status(403).json({
                    error: "You are not authorized to delete products. Only admins can.",
                });
            }
            // Check if the product with the given id exists
            const productId = req.params.id;
            config_1.default.query("SELECT * FROM products WHERE productCode = ?", [productId], (err, productResult) => {
                ...
                config_1.default.query("DELETE FROM products WHERE productCode = ?", [productId], (err, result) => {
                    if (err) {
                        return resp
                            .status(500)
                            .json({ error: "Database error", details: err });
